...
export class ProtocolKeyDefined<T, F extends ITypeBuilder<T, F>> {
    ...
    private readonly keyConsumer: (builder: F, value: unknown) => F;

    constructor(name: string, key: number, typeUid: string, keyConsumer: (builder: F, value: unknown) => F) {
        this.name = name;
        this.key = key;
     ...
     * @param property
     */
    apply(builder: F, property: unknown): F {
