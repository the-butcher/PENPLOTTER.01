...
export class CodedInputStream {
  ...
  readDouble(): number {
    ...
    const dataBuff: ArrayBuffer = new ArrayBuffer(8);
    const dataView: DataView = new DataView(dataBuff);
    for (let i = 0; i < 8; i++) {
  ...
  readString(): string {
    ...
    const stringByteCount: number = this.readRawVarint32();
    const iA = this.totalBytesRead;
    const iB = iA + stringByteCount;
